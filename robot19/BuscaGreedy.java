...
public class BuscaGreedy {
    ...
    private GraficoMapGreedy canvas;
    private int seq;
     ...
     */
    public BuscaGreedy(GridMap map, Block start, Block end) {
        this.map = map;
        this.start = start;
        this.end = end;
        this.canvas = new GraficoMapGreedy(map);
        canvas.desenha();
    }
...
//Limpa a fronteira e insere o nó raiz
        ...
        Comparator<Node> comparator = new ComparadorGreedy();
        ...
        while(!border.isEmpty()) {
            ...
            if(!stack.contains(atual)){
                ...
                if(atual.row == end.row && atual.col == end.col) {
                    canvas.drawBlock(atual, Color.RED);
                    return node;
                    ...
                    if(proximo != null && proximo.type != BlockType.WALL) {
                        ...
                        novoNode.setH(Math.abs(end.col - proximo.col) + Math.abs(end.row - proximo.row));
                        border.add(novoNode);
                        if (!stack.contains(proximo)){
                            canvas.drawBlock(proximo, Color.GREEN);
                        }
                    }
                }
                canvas.drawBlock(atual, Color.RED);
                canvas.saveFile("Greedy" + seq + ".png");
                seq++;
            }
