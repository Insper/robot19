...
//BufferedImage imagem;
     ...
     */
    public GraficoMapGreedy (GridMap g){
        this(g, 800, 600 );
    }
     ...
     */
    public GraficoMapGreedy(GridMap g, int width, int height){
        this.map = g;
        this.width = width;
        this.height = height;
    ...
    public void drawBlock(Block block, Color color){
        ...
        int dx = this.width/w;
        int dy = this.height/h;

        screen.setForegroundColor(color);
        screen.fillCircle(block.col*dx+dx/4, block.row*dy+dy/4, dx/2);
...
// Rodando a busca
        BuscaGreedy busca = new BuscaGreedy(map, sb, g);
